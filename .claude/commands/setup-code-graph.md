Set up code-graph MCP for the current project. This configures the MCP server, permissions, auto-sync hooks, and CLAUDE.md instructions.

Execute these steps in order:

## Step 1: Configure MCP server in ~/.claude.json

Run: `bash .claude/scripts/setup-code-graph.sh --mcp`
...
Run: `bash .claude/scripts/setup-code-graph.sh --permissions`

Confirm permissions were added to `.claude/settings.local.json`.

## Step 3: Ask about initializing the current project

Check if `.code-graph/graph.db` exists in the current project directory.

If it does NOT exist, ask the user: "Do you want to initialize code-graph for this project now? This will do a full index of the codebase."

If they say yes, run: `bash .claude/scripts/setup-code-graph.sh --init`
...
Run: `bash .claude/scripts/setup-code-graph.sh --hook`
...
Ask the user: "Where should I write the code-graph instructions — project CLAUDE.md or global ~/.claude/CLAUDE.md?"
...
This project uses code-graph MCP for codebase navigation. Available tools:
...
The index auto-syncs after git commits. To manually re-index: `java -jar /home/kamil/Documents/Project/My/code-graph/build/libs/code-graph-0.1.0.jar sync .`
```
