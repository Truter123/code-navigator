  ...
  template: `
    ...
    <div class="explorer-layout">
      <div class="memory-list-panel">
        <div class="filters">
          <select class="filter-select" [(ngModel)]="selectedAgent" (ngModelChange)="loadMemories()">
            <option value="">All Agents</option>
            @for (agent of agents; track agent.name) {
              <option [value]="agent.name">{{ agent.name }}</option>
            }
          </select>
          <input class="filter-input" placeholder="Search keys..." [(ngModel)]="searchQuery" (input)="filterMemories()">
        </div>
          ...
          @for (mem of filteredMemories; track mem.id) {
            <div class="memory-item" [class.selected]="selectedMemory?.id === mem.id" (click)="selectMemory(mem)">
              ...
              <div class="memory-meta">
                <span class="memory-agent">{{ mem.agentName }}</span>
      ...
      <div class="version-panel">
        @if (selectedMemory) {
          ...
          <div class="empty">Select a memory to view versions</div>
        }
      </div>
    </div>
  `,
...
export class MemoryExplorerComponent implements OnInit {
  ...
  loadMemories() {
    ...
    this.api.getMemories(params).subscribe(m => {
      this.memories = m;
      this.filterMemories();
    });
  }
